...
title = f'Karinn AI {fooocus_version.version}'
...
with shared.gradio_root:
    ...
    with gr.Row():
                ...
                with gr.Tabs(selected=modules.config.default_selected_image_input_tab_id):
                    ...
                    with gr.Tab(label='Describe', id='describe_tab') as describe_tab:
                        with gr.Row():
                            ...
                            with gr.Column():
                                interrogator_selection = gr.Radio(
                                    label='Interrogator',
                                    choices=['Photo (BLIP)', 'Anime (DeepBooru)'],
                                    value='Photo (BLIP)')
        ...
        def trigger_describe(interrogator, img, apply_styles):
            describe_prompts = []
            styles = set()

            if interrogator == 'Photo (BLIP)':
                from extras.interrogate import default_interrogator as default_interrogator_photo
                describe_prompts.append(default_interrogator_photo(img))
                styles.update(["Fooocus V2", "Fooocus Enhance", "Fooocus Sharp"])

            if interrogator == 'Anime (DeepBooru)':
                from extras.interrogate import default_anime_interrogator
                describe_prompts.append(default_anime_interrogator(img))
        ...
        describe_btn.click(trigger_describe, inputs=[interrogator_selection, describe_input_image, describe_apply_styles],
                           outputs=[prompt, style_selections], show_progress=True, queue=True) \
            .then(fn=style_sorter.sort_styles, inputs=style_selections, outputs=style_selections, queue=False, show_progress=False) \
            .then(lambda: None, _js='()=>{refresh_style_localization();}')

        if args_manager.args.enable_auto_describe_image:
            def trigger_auto_describe(interrogator, img, prompt, apply_styles):
                # keep prompt if not empty
                if prompt == '':
                    return trigger_describe(interrogator, img, apply_styles)
                return gr.update(), gr.update()

            uov_input_image.upload(trigger_auto_describe, inputs=[interrogator_selection, uov_input_image, prompt, describe_apply_styles],
                                   outputs=[prompt, style_selections], show_progress=True, queue=True) \
                .then(fn=style_sorter.sort_styles, inputs=style_selections, outputs=style_selections, queue=False, show_progress=False) \
                .then(lambda: None, _js='()=>{refresh_style_localization();}')

            enhance_input_image.upload(lambda: gr.update(value=True), outputs=enhance_checkbox, queue=False, show_progress=False) \
                .then(trigger_auto_describe, inputs=[interrogator_selection, enhance_input_image, prompt, describe_apply_styles],
                      outputs=[prompt, style_selections], show_progress=True, queue=True) \
                .then(fn=style_sorter.sort_styles, inputs=style_selections, outputs=style_selections, queue=False, show_progress=False) \
                .then(lambda: None, _js='()=>{refresh_style_localization();}')
